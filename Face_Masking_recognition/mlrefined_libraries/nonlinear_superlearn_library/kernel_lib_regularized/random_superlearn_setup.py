...
class Setup:
    ...
    def choose_kernel(self,D,**kwargs):    
        # setup kernel
        self.D = D
        N,P = self.x.shape
        w_internal = np.random.randn(N,D)
        self.H = lambda x,w=w_internal: np.cos(np.dot(w.T,x))
        self.H_train = self.H(self.x_train)
    ...
    def choose_cost(self,name,**kwargs):
        ...
        self.model = lambda x,w: w[0] + np.dot(self.H(x).T,w[1:])
        ...
        self.initializer = lambda: scale*np.random.randn(self.D + 1,dim)
