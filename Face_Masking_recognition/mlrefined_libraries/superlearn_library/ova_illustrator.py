...
class Visualizer:
    ...
    def solve_2class_subproblems(self, **kwargs):
        ...
        for i in range(0, num_classes):
            ...
            if algo == 'gradient_descent':  # run gradient descent
                w_hist = self.opt.gradient_descent(g=self.g, w=np.random.randn(np.shape(self.x)[1] + 1, 1),
                                                   version=version, max_its=max_its, alpha=alpha,
                                                   steplength_rule=steplength_rule)
            elif algo == 'newtons_method':
                w_hist = self.opt.newtons_method(g=self.g, w=np.random.randn(np.shape(self.x)[1] + 1, 1),
                                                 max_its=max_its, epsilon=10 ** (-5))
    ...
    def point_and_projection(self, point1, point2):
        ...
        for i in ind:
            ...
            # projected point
            ax.scatter(proj_point[0], proj_point[1], c=self.colors[i], edgecolor='k', linewidth=1, s=60, zorder=4,
                       marker='X')
        ...
        for i in ind:
            ...
            # projected point
            ax2.scatter(proj_point[0], proj_point[1], c=self.colors[i], edgecolor='k', linewidth=1, s=60, zorder=4,
                        marker='X')
