...
class Visualizer:
    ...
    def show_discrete_step(self, w_hist, view, **kwargs):
        ...
        for step in steps:
            ...
            ax.plot_surface(w1_vals, w2_vals, g_copy, alpha=0.25, color='w', zorder=0, edgecolor='k', linewidth=0.25,
                            cstride=200, rstride=200, shade=10, antialiased=True)
        ...
        if zplane == 'on':
            ...
            for c in class_nums:
                ...
                ax.contourf(w1_vals, w2_vals, g_vals - 0.5 - c, colors=self.colors[(int(c)):], alpha=0.1,
                            levels=range(0, 2))
        ...
        for c in range(C):
            ...
            ind = [v[0] for v in ind]
            ax.scatter(self.x[0, ind], self.x[1, ind], self.y[ind], s=80, color=self.colors[c], edgecolor='k',
    ...
    def compare_to_counting(self, **kwargs):
        ...
        for j in range(num_runs):
            ...
            if algo == 'gradient_descent':  # run gradient descent
                w_hist = self.opt.gradient_descent(g=g, w=w_init, version=version, max_its=max_its, alpha=alpha,
