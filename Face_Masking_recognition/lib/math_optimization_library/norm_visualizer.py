...
class visualizer:
        ...
        def animate(t):
            ...
            if k > 0:
                ...
                for j in range(min(k - 1, len(self.w_hist))):
                    w_val = self.w_hist[j]
                    g_val = self.g(w_val)
                    ax.scatter(w_val, g_val, s=90, c=self.colorspec[j], edgecolor='k',
                               linewidth=0.5 * ((1 / (float(j) + 1))) ** (0.4), zorder=3,
                               marker='X')  # plot point of tangency
                    ax.scatter(w_val, 0, s=90, facecolor=self.colorspec[j], edgecolor='k',
                               linewidth=0.5 * ((1 / (float(j) + 1))) ** (0.4), zorder=2)
            ...
            if k > 0 and k < len(self.w_hist) + 1:
                ...
                # plot tangent point
                ax.scatter(w, g_eval, s=100, c='m', edgecolor='k', linewidth=0.7, zorder=3,
                           marker='X')  # plot point of tangency
                ...
                if np.mod(t, 2) == 0:
                    ...
                    ax.scatter(w_zero, g_zero, s=100, c='m', edgecolor='k', linewidth=0.7, zorder=3,
                               marker='X')  # plot point of tangency
    ...
    def visualize3d(self, g, w_init, steplength, max_its, **kwargs):
        ...
        for k in range(len(self.w_hist)):
            w_now = self.w_hist[k]
            ax.scatter(w_now[0], w_now[1], 0, s=60, c=colorspec[k], edgecolor='k',
                       linewidth=0.5 * math.sqrt((1 / (float(k) + 1))), zorder=3)

            ax2.scatter(w_now[0], w_now[1], s=60, c=colorspec[k], edgecolor='k',
                        linewidth=1.5 * math.sqrt((1 / (float(k) + 1))), zorder=3)
        ...
        if len(self.w_hist) < 10:
            for i in range(len(self.w_hist) - 1):
                ...
                ax2.arrow(pt1[0], pt1[1], (pt2[0] - pt1[0]) * 0.78, (pt2[1] - pt1[1]) * 0.78, head_width=0.1,
                          head_length=0.1, fc='k', ec='k', linewidth=3, zorder=2, length_includes_head=True)
