    ...
    def plot_cost_histories(self,train_cost_histories,valid_cost_histories,start):        
        ...
        for c in range(len(train_cost_histories)):
            ...
            if len(valid_cost_histories) > 0:
                valid_history = valid_cost_histories[c]

                # plot valid cost function history
                ax.plot(np.arange(start,len(valid_history),1),valid_history[start:],linewidth = 3*(0.8)**(c),color = self.colors[1],label = 'valid_cost') 
    ...
    def plot_cost_count_histories(self,train_cost_histories,train_count_histories,valid_cost_histories,valid_count_histories,start):        
        ...
        for c in range(len(train_cost_histories)):
            ...
            if len(valid_cost_histories) > 0:
                ...
                ax2.plot(np.arange(start,len(valid_count_history),1),valid_count_history[start:],linewidth = 3*(0.8)**(c),color = self.colors[1],label = 'valid') 
        ...
        anchor = (1,1)
        plt.legend(loc='upper right', bbox_to_anchor=anchor)
        ax1.set_xlim([start - 0.1,len(train_cost_history) - 0.1])
        ax2.set_xlim([start - 0.1,len(train_cost_history) - 0.1])
        plt.show()       
