...
class Setup:
    def __init__(self,train_cost_histories,train_count_histories,valid_cost_histories,valid_count_histories,start,**kwargs):
        self.labels = [[] for v in range(len(train_cost_histories))]
        if 'labels' in kwargs:
            self.labels = kwargs['labels']
        
        # plotting colors
        self.colors = [[0,0.7,1],[1,0.8,0.5]]
        self.run_colors = ['black','magenta','lime','cyan'] 
    ...
    def plot_cost_histories(self,train_cost_histories,valid_cost_histories,start):        
        ...
        for c in range(len(train_cost_histories)):
            ...
            ax.plot(np.arange(start,len(train_history),1),train_history[start:],linewidth = 3*(0.8)**(c),color = self.colors[0],label = self.labels[c]) 
            ...
            if np.size(valid_cost_histories) > 0:
                ...
                ax.plot(np.arange(start,len(val_history),1),val_history[start:],linewidth = 3*(0.8)**(c),color = self.colors[1],label = 'valid cost')
    ...
    def plot_cost_count_histories(self,train_cost_histories,train_count_histories,valid_cost_histories,valid_count_histories,start):        
        ...
        for c in range(len(train_cost_histories)):
            ...
            ax1.plot(np.arange(start,len(train_cost_history),1),train_cost_history[start:],linewidth = 3,color = self.run_colors[c]) 
            #ax1.plot(np.arange(start,len(train_cost_history),1),train_cost_history[start:],linewidth = 1.5,color = self.colors[0]) 
  
            ax2.plot(np.arange(start,len(train_count_history),1),train_count_history[start:],linewidth = 3,color = self.run_colors[c],label = self.labels[c]) 
