...
class visualizer:
    ...
    def draw_it(self,**args):
        ...
        v = g1_plot[0]
        newplot = [v]
        for i in range(1,np.size(g1_plot)):
            pt = g1_plot[i]
            v = gamma*v + (1-gamma)*pt
            newplot.append(v)
        g1_plot = np.array(newplot)            
        ...
        def animate(k):
            ...
            ax1.set_title(title1,fontsize = 25,y=1.02)

            # plot function 2
            ax2.plot(w_plot,g2_plot,color = 'k',zorder = 1)                
            ax2.set_title(title2,fontsize = 25,y=1.02)
            ...
            ax3.set_title(title3,fontsize = 25,y=1.02)
            
            # find and plot min
            ind = np.argmin(g_combo)
            g_best = g_combo[ind]
            w_best = w_plot[ind]
            ax3.scatter(w_true,g_true,c = 'lime',edgecolor = 'k',zorder = 3,s = 100)
            ax3.scatter(w_best,g_best,c = 'red',edgecolor = 'k',zorder = 3,s = 100)
